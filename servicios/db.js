// ═══════════════════════════════════════════════════════════════════
// db.js — Turso via Cloudflare Worker (token NUNCA expuesto)
...
function getUID() {
  try {
    return window.__snapbookUser?.uid ?? null;
  } catch { return null; }
}
...
async function exec(sql, args = []) {
  ...
  if (!res.ok) {
    ...
    throw new Error(`Worker HTTP ${res.status}: ${txt}`);
  }
...
async function batch(statements) {
  ...
  if (!res.ok) throw new Error(`Worker batch HTTP ${res.status}: ${await res.text()}`);
  return res.json();
}
...
async function initTables() {
  await batch([
    ...
    { sql: `CREATE TABLE IF NOT EXISTS likes_shorts (
    ...
    { sql: `CREATE TABLE IF NOT EXISTS visitas_perfil (
              id INTEGER PRIMARY KEY AUTOINCREMENT,
    ...
    { sql: `CREATE TABLE IF NOT EXISTS encuestas (
              ...
              timestamp INTEGER NOT NULL,
              uid TEXT NOT NULL)` },
    { sql: `CREATE INDEX IF NOT EXISTS idx_enc_post ON encuestas(post_id)` },
    { sql: `CREATE INDEX IF NOT EXISTS idx_enc_uid  ON encuestas(uid)` },

    { sql: `CREATE TABLE IF NOT EXISTS votos_encuesta (
              encuesta_id TEXT NOT NULL,
              opcion_idx  INTEGER NOT NULL,
              uid         TEXT NOT NULL,
              timestamp   INTEGER NOT NULL,
              PRIMARY KEY (encuesta_id, uid))` },
    { sql: `CREATE INDEX IF NOT EXISTS idx_votos_enc ON votos_encuesta(encuesta_id)` },
    ...
    { sql: `CREATE TABLE IF NOT EXISTS llamadas (
              ...
              timestamp INTEGER NOT NULL)` },
    { sql: `CREATE INDEX IF NOT EXISTS idx_llamadas_de   ON llamadas(de_uid)` },
    { sql: `CREATE INDEX IF NOT EXISTS idx_llamadas_para ON llamadas(para_uid)` },
    { sql: `CREATE INDEX IF NOT EXISTS idx_llamadas_chat ON llamadas(chat_id)` },
  ]);
...
export async function sincronizarUsuario(user) {
  ...
  // Si photoURL está vacío, no sobreescribir el avatar que ya existe en Turso
  if (photoURL) {
    await exec(
      ...
      [user.uid, user.displayName ?? "Usuario", photoURL, user.email ?? "", Date.now()]
    );
  } else {
    await exec(
      `INSERT INTO usuarios (uid, nombre, avatar, email, updated_at) VALUES (?, ?, ?, ?, ?)
       ON CONFLICT(uid) DO UPDATE SET nombre=excluded.nombre, email=excluded.email, updated_at=excluded.updated_at`,
      [user.uid, user.displayName ?? "Usuario", photoURL, user.email ?? "", Date.now()]
    );
  }
}
...
export async function toggleLike(postId, uid) {
  ...
  if (existe.length) {
    ...
    await exec("UPDATE posts SET total_likes = (SELECT COUNT(*) FROM likes WHERE post_id = ?) WHERE id = ?", [postId, postId]);
    return false;
  } else {
...
export async function crearHistoria(user, { imagenUrl = null, videoUrl = null, textoHistoria = null, bgGradient = null, timestamp, expira }) {
  ...
  await exec(
    ...
    [id, user.uid, user.displayName ?? "Usuario", user.photoURL ?? "",
     imagenUrl, videoUrl, textoHistoria, bgGradient, timestamp ?? Date.now(), expira]
  );
  return id;
}
...
export async function seguirUsuario(uid, seguidorUid) {
  await exec(
    "INSERT OR IGNORE INTO seguidores (uid, seguidor_uid, timestamp) VALUES (?, ?, ?)",
    [uid, seguidorUid, Date.now()]
  );
}
...
export async function obtenerSeguidores(uid) {
  return exec("SELECT seguidor_uid FROM seguidores WHERE uid = ?", [uid]);
}

export async function obtenerSeguidos(uid) {
  return exec("SELECT uid FROM seguidores WHERE seguidor_uid = ?", [uid]);
}
...
// VISITAS DE PERFIL
// ═══════════════════════════════════════════════════════════════════
export async function registrarVisitaPerfil(uid, visitorUid) {
  if (uid === visitorUid) return;
  await exec(
    "INSERT INTO visitas_perfil (uid, visitor_uid, timestamp) VALUES (?, ?, ?)",
    [uid, visitorUid, Date.now()]
  );
}

export async function obtenerVisitasPerfil(uid, limite = 50) {
  return exec(
    "SELECT * FROM visitas_perfil WHERE uid = ? ORDER BY timestamp DESC LIMIT ?",
    [uid, limite]
  );
}
...
export async function actualizarEstadoLlamada(firebaseId, estado) {
  await exec("UPDATE llamadas SET estado = ? WHERE firebase_id = ?", [estado, firebaseId]);
}
...
export async function obtenerFeedCompleto(uids = [], limite = 20, antes = Date.now(), uid = null) {
  ...
  // Recalcular total_likes desde la tabla real para evitar desincronización
  if (postIds.length) {
    const ph2     = postIds.map(() => '?').join(',');
    const counts  = await exec(
      `SELECT post_id, COUNT(*) as cnt FROM likes WHERE post_id IN (${ph2}) GROUP BY post_id`,
      postIds
    );
    const countMap = {};
    counts.forEach(r => { countMap[r.post_id] = r.cnt; });
    posts.forEach(p => { p.total_likes = countMap[p.id] ?? 0; });
  }
